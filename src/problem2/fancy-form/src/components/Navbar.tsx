...
const Navbar: React.FC<NavbarProps> = ({ balances, options }) => {
  ...
  return (
    <nav className="absolute text-sm overflow-x-scroll overflow-y-hidden top-0 left-0 xl:left-[25%] w-full xl:w-7/12 2xl:w-6/12 bg-zinc-800 p-4 flex justify-between items-center text-white rounded-md bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-20 border-0 border-b border-opacity-20 border-gray-100">
        ...
        {Object.entries(balances)
          ...
          .map(([currency, balance]) => (
            <div key={currency} className="flex items-center whitespace-nowrap">
              ...
              <span className="mr-1">
                {balance % 1 === 0 ? balance : balance.toFixed(7).replace(/\.?0+$/, "")}
              </span>
              <span>{currency}</span>
            </div>
          ))}
